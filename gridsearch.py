...
for lr, batch_size, dropout_rate in itertools.product(learning_rates, batch_sizes, dropout_rates):
    print(f"Running experiment with lr={lr}, batch_size={batch_size} ,dropout_rate={dropout_rate}")
    # Define experiment name based on the hyperparameters
    experiment_name = f"lr_{lr}_batch_{batch_size}_l1_dropout_{dropout_rate}"
    print(f"Running experiment: {experiment_name}")


    sketch_dir = "Data/raw/gamma_inverted_sketches"
    photo_dir =  "Data/raw/portraits"
    batch_size = batch_size
    num_epochs =  50
    lr = lr
    save_path = "savemodels/gamma"
    patience = 10
